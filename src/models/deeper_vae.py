...
"""
class deeper_VAE(nn.Module):
    def __init__(self, imgChannels=1, featureDim=4096, zDim=128):
        super(deeper_VAE, self).__init__()
       ...
       # Vae model made to match that in the Defence-VAE code base
        ...
        self.encConv2_deep = nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1, bias= False)
        self.encConv2_deep_bn = nn.BatchNorm2d(64)
        self.encConv3 = nn.Conv2d(64, 128, kernel_size=4, stride=2, padding=1, bias= False)
        self.encConv3_bn = nn.BatchNorm2d(128)
        self.encConv4_deep = nn.Conv2d(128, 128, kernel_size=3, stride=1, padding=1, bias= False)
        self.encConv4_deep_bn = nn.BatchNorm2d(128)
        ...
        self.deconv1_deep = nn.ConvTranspose2d(128, 128, kernel_size=3, stride=1, padding=1, bias= False)
        self.deconv1_bn_deep = nn.BatchNorm2d(128)
        ...
        self.deconv3_deep = nn.ConvTranspose2d(64, 64, kernel_size=3, stride=1, padding=1, bias= False)
        self.deconv3_bn_deep = nn.BatchNorm2d(64)
    ...
    def encoder(self, x):
        ...
        out = self.relu(self.encConv2_deep_bn(self.encConv2_deep(out)))
        out = self.relu(self.encConv2_deep_bn(self.encConv2_deep(out)))
        out = self.relu(self.encConv2_deep_bn(self.encConv2_deep(out)))
        out = self.relu(self.encConv2_deep_bn(self.encConv2_deep(out)))
        out = self.relu(self.encConv2_deep_bn(self.encConv2_deep(out)))
        ...
        out = self.relu(self.encConv4_deep_bn(self.encConv4_deep(out)))
        out = self.relu(self.encConv4_deep_bn(self.encConv4_deep(out)))
        out = self.relu(self.encConv4_deep_bn(self.encConv4_deep(out)))
        out = self.relu(self.encConv4_deep_bn(self.encConv4_deep(out)))
        out = self.relu(self.encConv4_deep_bn(self.encConv4_deep(out)))
    ...
    def decoder(self, z):
        ...
        out = self.relu(self.deconv1_bn_deep(self.deconv1_deep(out)))
        out = self.relu(self.deconv1_bn_deep(self.deconv1_deep(out)))
        out = self.relu(self.deconv1_bn_deep(self.deconv1_deep(out)))
        out = self.relu(self.deconv1_bn_deep(self.deconv1_deep(out)))
        out = self.relu(self.deconv1_bn_deep(self.deconv1_deep(out)))
        ...
        out = self.relu(self.deconv3_bn_deep(self.deconv3_deep(out)))
        out = self.relu(self.deconv3_bn_deep(self.deconv3_deep(out)))
        out = self.relu(self.deconv3_bn_deep(self.deconv3_deep(out)))
        out = self.relu(self.deconv3_bn_deep(self.deconv3_deep(out)))
        out = self.relu(self.deconv3_bn_deep(self.deconv3_deep(out)))
